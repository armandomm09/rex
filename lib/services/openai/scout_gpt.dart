...
class ScoutGPTService {
  ...
  static newGptMessage(String question) async {
    final GptMessage message = GptMessage(currentUserID, currentUserEmail,
        question, Timestamp.now(), const Uuid().v1());
...
>>>>>>> main
  ...
  static newGPTAnswer(String question) async {
    try {
      ...
      GptMessage loadingMessage = GptMessage('chatGPT', 'chatGPT@imperator.com',
          "-LOADING-", Timestamp.now(), loadingMessageUuid);
      await firestore
          .collection('Users')
          .doc(currentUserID)
          .collection('gptChat')
      ...
      GptMessage message = GptMessage('chatGPT', 'chatGPT@imperator.com',
          answer.trim(), Timestamp.now(), loadingMessageUuid);
